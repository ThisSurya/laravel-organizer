<x-app-layout>
    <x-slot name="navhead">
        <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
            <li class="text-sm leading-normal">
                <a class="text-white opacity-50" href="{{ route('posting.store') }}">{{ __('Main') }}</a>
            </li>
            <li class="text-sm pl-2 capitalize leading-normal text-white before:float-left before:pr-2 before:text-white before:content-['/']"
                aria-current="page">{{ __('Dashboard') }}</li>
        </ol>
        <h6 class="mb-0 font-bold text-white capitalize">{{ __('Dashboard') }}</h6>
    </x-slot>
    ...
    <div class="w-full px-6 py-6 mx-auto">
        ...
        <div class="flex flex-wrap -mx-3 h-screen">
            <!-- card1 -->
            <h1>Laravel ORG-Manager Dashboard</h1>
        </div>
